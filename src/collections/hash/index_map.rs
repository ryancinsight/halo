    ...
    S: BuildHasher,
{
    ...
    fn grow(&mut self, new_cap: usize) {
        ...
        // Rehash all existing items using iterator chains
        ...
        let items_count = &mut self.items_count;

        // Iterate over dense keys
        keys.iter().enumerate().for_each(|(i, key)| {
            // Local hash calculation to avoid borrowing self
            let mut hasher = hash_builder.build_hasher();
            key.hash(&mut hasher);
            let hash = hasher.finish();
            let h1 = (hash as usize) & (table_capacity - 1);
            let h2 = ((hash >> 57) as u8) & 0x7F;

            let mask = table_capacity - 1;

            // Custom probe iterator logic to avoid mutable state capture issues
            // (idx, step)
            let mut idx = h1;
            let mut step = GROUP_WIDTH;

            // We use `std::iter::from_fn` to generate probe indices
            let mut probe_iter = std::iter::from_fn(move || {
                let current_idx = idx;
                idx = (idx + step) & mask;
                step += GROUP_WIDTH;
                Some(current_idx)
            ...
            let found_slot = probe_iter.find_map(|probe_idx| {
                let group_word = unsafe {
                    let ptr = ctrl.as_ptr().add(probe_idx);
                    std::ptr::read_unaligned(ptr as *const u64)
                };
                ...
                if empty_mask != 0 {
                    let trailing = empty_mask.trailing_zeros() / 8;
                    let slot_idx = (probe_idx + trailing as usize) & mask;
                    Some(slot_idx)
                } else {
                    None
                }
            ...
            if let Some(slot_idx) = found_slot {
                unsafe {
                    *slots.get_unchecked_mut(slot_idx) = i;
                    ctrl[slot_idx] = h2;
                    if slot_idx < GROUP_WIDTH {
                        ctrl[table_capacity + slot_idx] = h2;
                    }
                }
                *items_count += 1;
            } else {
                // Should never happen during grow as we ensure capacity
                unreachable!("Table full during rehash");
            }
        });
    }
