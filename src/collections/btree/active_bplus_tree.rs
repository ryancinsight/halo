...
impl<'a, 'brand, K, V> ActiveBPlusTree<'a, 'brand, K, V> {
    pub fn new(tree: &'a mut BrandedBPlusTree<'brand, K, V>, token: &'a mut GhostToken<'brand>) -> Self {
        Self { tree, token }
    }
...
impl<'brand, K, V> BrandedBPlusTree<'brand, K, V> {
    pub fn activate<'a>(&'a mut self, token: &'a mut GhostToken<'brand>) -> ActiveBPlusTree<'a, 'brand, K, V> {
