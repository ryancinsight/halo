        ...
        loop {
            ...
            if child_vec_idx < node.children.len() {
                ...
                if let NodeSlot::Occupied(child_node) = child_slot {
                    Rc::make_mut(&mut self.key_buf).extend_from_slice(&child_node.prefix);
            ...
            if self.key_buf.len() >= popped_len {
                ...
                Rc::make_mut(&mut self.key_buf).truncate(new_len);
            }
        }
    }
}
